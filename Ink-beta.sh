...
show "Starting Deploy Ink..." "progress"
sleep 2
...
if [ -d "ink" ]; then
  rm -rf ink
  show "Removed existing Ink folder." "done"
fi
...
show "Setting up ink and OpenZeppelin..." "progress"
mkdir -p ink && cd ink
...
EOF
  ...
  DEPLOY_CMD="forge create winsnip/$CONTRACT_NAME.sol:$CONTRACT_NAME --constructor-args $YOUR_ADDRESS --rpc-url https://rpc-gel-sepolia.inkonchain.com --private-key $YOUR_PRIVATE_KEY"
  eval $DEPLOY_CMD
  ...
  for ((i=0; i<NUM_ADDRESSES; i++)); do
      ...
      SEND_CMD="cast send \"$CONTRACT_ADDRESS\" \"transfer(address,uint256)\" \"$RANDOM_ADDRESS\" \"$AMOUNT_WEI\" --rpc-url https://rpc-gel-sepolia.inkonchain.com --private-key \"$YOUR_PRIVATE_KEY\""
  ...
  show "https://explorer-sepolia.inkonchain.com/address/$YOUR_ADDRESS#tokentxns" "done"
...
EOF
  ...
  DEPLOY_CMD="forge create winsnip/MyNFT.sol:MyNFT --rpc-url https://rpc-gel-sepolia.inkonchain.com --private-key $YOUR_PRIVATE_KEY --constructor-args $OWNER_ADDRESS"
  eval $DEPLOY_CMD
  ...
  read -p "How many NFTs to mint: " NUM_NFTS
    ...
    for ((i=0; i<OWNER_MINT_COUNT; i++)); do
        ...
        MINT_CMD="cast send \"$NFT_CONTRACT_ADDRESS\" \"safeMint(address,string)\" \"$OWNER_ADDRESS\" \"$TOKEN_URI\" --rpc-url https://rpc-gel-sepolia.inkonchain.com --private-key \"$YOUR_PRIVATE_KEY\""
        eval $MINT_CMD
    done
    ...
    for ((i=0; i<RANDOM_MINT_COUNT; i++)); do
        ...
        MINT_CMD="cast send \"$NFT_CONTRACT_ADDRESS\" \"safeMint(address,string)\" \"$RANDOM_ADDRESS\" \"$TOKEN_URI\" --rpc-url https://rpc-gel-sepolia.inkonchain.com --private-key \"$YOUR_PRIVATE_KEY\""
        eval $MINT_CMD
    done
  show "NFT contract can be viewed at: https://explorer-sepolia.inkonchain.com/$NFT_CONTRACT_ADDRESS" "done"
